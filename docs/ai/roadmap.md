...
- [x] Entité `Lieu` + CRUD : `EmplacementLieu`, **partage entre activités** (`partageableEntreAnimateurs`, `nombreMaxActivitesSimultanees`), **usages multiples** (`UsageLieu` : `ACTIVITE` / `SURVEILLANCE` / `RASSEMBLEMENT`, table `lieu_usage`, règles **activités** vs **planning** via `LieuUsageRules`), `SaveLieuRequest` / `LieuDto`, `LieuService`/`LieuServiceImpl`, `LieuController`, `LieuControllerTest` (7), `LieuServiceImplTest` (**9**). ⚠️ Front : **`api.d.ts`** + formulaires lieu avec **`usages`**. ✅
