...
- **`HistoriqueModification`** : Traçabilité des modifications sur activités et cellules de planning **avec capture automatique des valeurs**. Table **`historique_modification`** avec stratégie **`@Inheritance(SINGLE_TABLE)`** + **`@DiscriminatorColumn(name="type")`**. Champs communs : **`id`**, **`action`** (**`HistoriqueModificationAction`** : **`CREATION`**, **`MODIFICATION`**, **`SUPPRESSION`**), **`dateModification`** (`Instant`), **`modificateur`** (**`@ManyToOne` obligatoire vers `Utilisateur`**), **`ancienne_valeur`** (TEXT, libellés lisibles avant modification), **`nouvelle_valeur`** (TEXT, libellés lisibles après modification). Index : **`idx_hist_mod_ligne_jour`** (`planning_ligne_id`, `planning_jour`, `date_modification`), **`idx_hist_mod_activite`** (`activite_id`, `date_modification`). Sous-classes : **`HistoriqueModificationActivite`** (colonnes **`activite_id`** nullable — SINGLE_TABLE), **`HistoriqueModificationPlanningCellule`** (colonnes **`planning_ligne_id`**, **`planning_jour`** `LocalDate`, **`planning_cellule_id`** — toutes nullables pour SINGLE_TABLE). Enum **`HistoriqueModificationType`** pour DTOs (**`ACTIVITE`**, **`PLANNING_CELLULE`**). **`HistoriqueModificationRepository`** : requêtes **`findActiviteByActiviteId`**, **`findPlanningByLigneId`**, **`findPlanningByLigneIdAndJour`** (type + tri chronologique). **Capture pour l’historique (libellés)** : **`ActiviteServiceImpl.activiteLibellePourHistorique`** (via **`snapshotActivite`**) ; **`PlanningGrilleServiceImpl.contenuCelluleLibellePourHistorique`** (via **`snapshotPlanningCellule`**). **Détection de changement** : **`signatureActivite`**, **`signatureContenuCellule`** (ids / tokenIds). **Logique valeurs** : création (null, nouvelle valeur), modification (ancienne valeur, nouvelle valeur), suppression (ancienne valeur, null). Enregistrement : **`ActiviteServiceImpl`** (POST/PUT/DELETE avec snapshots), **`PlanningGrilleServiceImpl.remplacerCellules`** (signature avant/après pour détecter vraie modification + snapshots) et **`modifierMaPresenceSurCelluleMembreEquipe`** (PATCH ma-présence, snapshots ciblés). Consultation : **`GET .../activites/{id}/historique`** et **`GET .../planning-grilles/.../lignes/{id}/historique-cellules?jour={jour}`** avec **`ACCES_SEJOUR`** + appartenance.
