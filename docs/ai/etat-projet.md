...
- [FAIT] **`ChambreControllerTest`** ✅
  - **7** tests : `GET .../chambres` (200), `GET .../chambres/{id}` (200/404), `POST` (201/409 identifiant), `PUT` (200), `DELETE` (204)
  - `MockMvc` standalone + `GlobalExceptionHandler`, même remarque sur `@PreAuthorize` / `@Valid`

- [FAIT] **`ChambreServiceImplTest`** ✅
  - **10** tests : doublon identifiant (casse), création trim, chambre équipe, modifier conflit identifiant, passage ENFANT→EQUIPE (efface référents), même identifiant casse différente, lister 404/vide, ajout référent OK, refus référent sur EQUIPE, doublon référent
...
- **`Chambre`** : Hébergement pour un **`Sejour`** (**`ManyToOne`** obligatoire), **distinct de `Lieu`**. Champs : **`typeChambre`** (`TypeChambre` : **`ENFANT`** / **`EQUIPE`**), **`identifiant`** obligatoire (**`uk_chambre_sejour_identifiant`**, unicité par séjour, casse ignorée côté service), **`nom`** optionnel (surnom), **`capaciteMax`**, **`genreAutorise`** (`GenreChambre`), **`description`**, **`batiment`**, **`couloir`**, **`etage`**. **`@ManyToMany`** référents (**`chambre_referent`**) — **`ENFANT`** only en API ; **`EQUIPE`** : pas de référents. **`Sejour.chambres`** **`OneToMany`** ; **`Utilisateur.chambresReferent`**. **`ChambreRepository`** : **`findBySejourIdOrderAffichage`**, **`existsBySejourIdAndIdentifiantIgnoreCase*`**. API **`/api/v1/sejours/{sejourId}/chambres`**. **Hors périmètre** : affectation occupants. Tests : **`ChambreControllerTest`** (**7**), **`ChambreServiceImplTest`** (**10**). **jour + moment** (voir comptages activités).
