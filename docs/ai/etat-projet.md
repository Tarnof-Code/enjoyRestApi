...
- [FAIT] **`ChambreServiceImplTest`** ✅
  - **26** tests : doublon identifiant (casse), création trim, chambre équipe, création avec **`groupeId`**, modifier conflit identifiant, passage ENFANT→EQUIPE (efface référents), changement de type (efface occupants), genre incompatible avec occupants (enfant/équipe), même identifiant casse différente, lister 404/vide, ajout référent OK, refus référent sur EQUIPE, doublon référent, affectation enfant/membre (succès, genre, capacité, type chambre, inscription, groupe), batch enfants/équipe, retrait enfant
